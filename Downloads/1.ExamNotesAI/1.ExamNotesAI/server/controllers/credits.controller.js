...
// Stripe is optional for local dev — graceful fallback if key not set
const stripeKey = process.env.STRIPE_SECRET_KEY
const stripe = stripeKey && !stripeKey.includes("Enter") ? new Stripe(stripeKey) : null
...
export const stripeWebhook = async (req,res) => {
    if (!stripe) return res.status(503).json({ message: "Webhook not configured" });
